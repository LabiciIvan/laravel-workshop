        ...
        <div class="flex items-center">
          <div class="hidden md:block">
            <nav class="ml-10 flex items-baseline space-x-4">
                <x-nav-link path="/" name="Home" :active="request()->is('/')"/>
                <x-nav-link path="/contact" name="Contact" :active="request()->is('contact')" />
                <x-nav-link path="/about" name="About" :active="request()->is('about')"/>
                <x-nav-link path="/jobs" name="Jobs" :active="request()->is('jobs')"/>
                <x-nav-link path="/jobs/create" name="Create Job" :active="request()->is('jobs/create')"/>
            </nav>
          </div>
        </div>
        <div class="hidden md:block">
          <div class="ml-4 flex items-center md:ml-6">
            @if (Auth::check())
            <form method="POST" action="{{route('login.logout', ['id' => Auth::user()->id])}}">
              @csrf
                {{-- <x-nav-link path="/logout/{{Auth::user()->id}}" name="Logout" :active="request()->is('logout')"/> --}}
                  <x-form-button type="submit" innerHtml="Logout" class="bg-red-600 hover:bg-red-700" />
            </form>
            @else
              <x-nav-link path="/login" name="Login" :active="request()->is('login')"/>
              <x-nav-link path="/register" name="Register" :active="request()->is('register')"/>
            @endif
